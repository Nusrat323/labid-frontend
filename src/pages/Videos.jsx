...
export default function Videos() {
  ...
  const showNext = () => setLightboxIndex((prev) => (prev === filteredVideos.length - 1 ? 0 : prev + 1));
  ...
  return (
    <div className="min-h-screen pt-32 pb-10 px-6 bg-gradient-to-b from-stone-800 via-stone-700 to-stone-600 text-white">
      ...
      <div className={`grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 transition-opacity duration-500 ${fade ? "opacity-100" : "opacity-0"}`}>
        {currentVideos.map((video, idx) => (
          <div
            key={idx}
            className="relative rounded-xl overflow-hidden shadow-lg hover:shadow-2xl cursor-pointer"
            onClick={() => openLightbox(indexOfFirstVideo + idx)}
          >
            <video
              src={video.src}
